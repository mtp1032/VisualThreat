...
-- next frame.
local frameRate = (1/GetFramerate())
if frameRate < 0.01 then
    frameRate = 0.01
end

local _DEFAULT_TIMER_INTERVAL   = frameRate
...
function timer:yield()
    ...
    h[TH_YIELD_TIME]  = h[TH_YIELD_TIME] + (elapsedTime)/1000
...
function timer:getMetrics( thread_h )
    ...
    local avgYieldTime = (thread_h[TH_YIELD_TIME] / thread_h[TH_YIELD_COUNT]) 
    ...
    if totalLifetime > 0 then
        avgTicksPerYield = avgYieldTime / timerDispatchInterval
        yieldRatio = avgYieldTime / totalLifetime 
        runRatio = 1 - yieldRatio
    end

    return threadState, avgYieldTime, avgTicksPerYield, totalLifetime, yieldRatio, runRatio
end
